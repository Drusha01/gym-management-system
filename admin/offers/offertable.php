            ...
            if($offers_data = $offersObj->fetch()){
                ...
                foreach ($offers_data as $key => $value) {
                    if($value['status_details'] =='active'){
                        ...
                        echo '<td class="text-center "><a href="editoffer.php?id='; echo_safe($value['offer_id']); echo'" class="btn btn-primary btn-sm" role="button">Edit</a> <button href="deleteoffer.php?id='; echo_safe($value['offer_id']); echo'" class="btn btn-danger btn-sm" onclick="confirmfunction(';echo $counter; echo')">Delete</button></td>';
                        echo '</tr>';
                        $counter++;
                    }
                }
