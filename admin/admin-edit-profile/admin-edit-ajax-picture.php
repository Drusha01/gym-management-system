...
if(isset($_SESSION['admin_id'])){
    ...
    if($_SESSION['admin_user_status_details'] == 'active'){
        ...
        if(isset($_POST['user_id'])){
            $OlduserObj = new users();
            $user_id = $_POST['user_id'];
            $OlduserObj->setuser_id($user_id);

            $old_user_data = $OlduserObj->get_user_details();
            ...
            if($profile_pic || $valid_id){
                ...
                if($valid_id){
                    if($userObj->update_valid_id()){
                        ...
                        if($old_user_data['user_valid_id_photo'] != 'default.png' && unlink(dirname(__DIR__, 2) . '/img/valid-id/'.$old_user_data['user_valid_id_photo'])){
                            // update session
                ...
                if($profile_pic){
                    if($userObj->update_profile_pic()){
                        ...
                        // delete the old file
                        if ( $old_user_data['user_profile_picture'] != 'default.png' && unlink(dirname(__DIR__, 2) . '/img/profile/' . $old_user_data['user_profile_picture']) && unlink(dirname(__DIR__, 2) . '/img/profile-resize/' . $old_user_data['user_profile_picture']) && unlink(dirname(__DIR__, 2) . '/img/profile-thumbnail/' . $old_user_data['user_profile_picture'])) {
                            // update session
