...
class trainers
{
    ...
    function fetch_trainer_with_id($trainer_id){
        try{
            ...
            user_phone_number,user_email,user_profile_picture,trainer_status_description,user_valid_id_photo,user_date_created FROM trainers
            LEFT OUTER JOIN users ON trainers.trainer_user_id=users.user_id
            LEFT OUTER JOIN user_genders ON users.user_gender_id=user_genders.user_gender_id
            LEFT OUTER JOIN trainer_availability ON trainers.trainer_availability_id=trainer_availability.trainer_availability_id
            LEFT OUTER JOIN user_status ON users.user_status_id=user_status.user_status_id
    ...
    function update_trainer_status_description($trainer_id,$trainer_user_id,$trainer_status_description){
        try{
            $sql = 'UPDATE trainers 
            SET trainer_status_description = :trainer_status_description
            WHERE trainer_id = :trainer_id AND trainer_user_id = :trainer_user_id;';
            $query=$this->db->connect()->prepare($sql);
            $query->bindParam(':trainer_id', $trainer_id);
            $query->bindParam(':trainer_user_id', $trainer_user_id);
            $query->bindParam(':trainer_status_description', $trainer_status_description);
            return $query->execute();
        }catch (PDOException $e){
            return false;
        }
